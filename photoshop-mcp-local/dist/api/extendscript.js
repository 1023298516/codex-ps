...
export const ExtendScriptSnippets = {
  ...
  `,
     ...
     */
    newDocument: (width, height, resolution = 72, colorMode = 'NewDocumentMode.RGB') => `
  ...
  `,
     ...
     * Create a text layer
     */
    createTextLayer: (text, x = 100, y = 100, fontSize = 24) => `
    ${getContextInfo}
  ...
  `,
     ...
     */
    scaleLayer: (scalePercent, centerAnchor = true) => `
    if (app.documents.length === 0) {
      throw new Error('No active document');
    }
    var doc = app.activeDocument;
  ...
  `,
     ...
     */
    adjustHueSaturation: (hue, saturation, lightness) => `
    ${helperFunctions}
  ...
  `,
     ...
     * Crop document
     */
    cropDocument: (left, top, right, bottom) => `
    if (app.documents.length === 0) {
      throw new Error('No active document');
    }
    var doc = app.activeDocument;
  ...
  `,
     ...
     */
    selectRectangle: (left, top, right, bottom) => `
    if (app.documents.length === 0) {
      throw new Error('No active document');
    }
    var doc = app.activeDocument;
  ...
  `,
     ...
     */
    moveLayerToPosition: (targetLayerName, position) => `
    ${getContextInfo}
  ...
  `,
    ...
    if (doc.layers.length > 0) {
      ...
      if (bottomLayer.isBackgroundLayer) {
        ...
        layer.move(bottomLayer, ElementPlacement.PLACEAFTER);
      }
    }
 ...
 */
export function generateExtendScript(code) {
    return `
(function() {
  try {
    ${code}
...
//# sourceMappingURL=extendscript.js.map
