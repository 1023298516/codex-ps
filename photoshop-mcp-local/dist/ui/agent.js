    ...
    try {
        ...
        for await (const part of result.fullStream) {
            switch (part.type) {
                ...
                case 'tool-error': {
                    const tc = buffer.toolCalls.find((c) => c.id === part.toolCallId);
                    const text = part.error?.message ?? String(part.error);
                    if (tc) {
                ...
                case 'error': {
                    yield {
                        type: 'error',
                        payload: { message: part.error?.message ?? String(part.error) },
                    };
                    break;
                }
                default:
...
export function computeCost(usage, pricing) {
    ...
    const cachedReadUsd = (cacheRead / 1_000_000) * (pricing.cachedInputUsdPerMTok ?? pricing.inputUsdPerMTok);
    const cachedWriteUsd = (cacheWrite / 1_000_000) * (pricing.cachedWriteUsdPerMTok ?? pricing.inputUsdPerMTok);
    return {
