...
export class MacOSDetector {
    ...
    async detectUsingSpotlight() {
        try {
            // Use mdfind to search for Photoshop applications
            const { stdout } = await execAsync('mdfind "kMDItemCFBundleIdentifier == com.adobe.Photoshop"');
            const apps = stdout
                .split('\n')
    ...
    getCommonPaths() {
        ...
        for (let year = 2025; year >= 2012; year--) {
            paths.push(`/Applications/Adobe Photoshop ${year}/Adobe Photoshop ${year}.app`, `/Applications/Adobe Photoshop CC ${year}/Adobe Photoshop CC ${year}.app`, `/Applications/Adobe Photoshop ${year}.app`);
        }
        // Also check for version-less installation
        paths.push('/Applications/Adobe Photoshop CC/Adobe Photoshop CC.app', '/Applications/Adobe Photoshop/Adobe Photoshop.app', '/Applications/Adobe Photoshop.app');
        return paths;
    }
    ...
    async extractVersionFromApp(appPath) {
        try {
            ...
            try {
                ...
                const { stdout: version } = await execAsync(`/usr/libexec/PlistBuddy -c "Print :CFBundleShortVersionString" "${plistPath}"`);
                if (version.trim()) {
                    return version.trim();
                }
            }
...
//# sourceMappingURL=macos-detector.js.map